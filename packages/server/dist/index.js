...
var useEditorStore = create((set, get, store) => ({
  ...
  setDirection: (dir) => set({ direction: dir, consumed: false, canvasSource: "user" }),
  // 消费状态操作
...
async function startServer(options = {}) {
  ...
  app.post("/mcp", async (req, res) => {
    console.log("[MCP] \u6536\u5230 POST \u8BF7\u6C42, Content-Type:", req.headers["content-type"], "Accept:", req.headers["accept"]);
    try {
      await transport.handleRequest(req, res);
      console.log("[MCP] \u8BF7\u6C42\u5904\u7406\u5B8C\u6210, status:", res.statusCode);
    } catch (err) {
  ...
  app.get("/health", (req, res) => {
    res.json({ status: "ok", service: "mermaid-editor" });
  });
