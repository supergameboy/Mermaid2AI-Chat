...
allowed-tools: mcp__mermaid-editor__get_input, mcp__mermaid-editor__create_view
version: 1.1.0
...
用户输入 `/mermaid-flow-editor` 时，默认调用 `get_input` 读取画布（get_input 是核心功能）。
...
Mermaid 反向编辑器的 AI 侧入口。连接用户可视化编辑器与 AI 对话，通过 MCP 协议消除"编辑器↔AI对话"的复制粘贴割裂感。
...
| 主动调用 | 用户输入 `/mermaid-flow-editor` 默认读图 |

### 依赖

| 依赖 | 说明 |
|------|------|
| MCP 服务端 `mermaid-editor` | 提供 `get_input` 和 `create_view` 工具 |
