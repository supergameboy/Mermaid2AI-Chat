...
function fixUnsupportedNodeShapes(source, astNodes) {
    ...
    for (const [originalId, astNode] of astNodes) {
        ...
        for (const line of lines) {
            ...
            // 检查是否以节点 ID 开头
            if (!trimmed.startsWith(originalId))
                continue;
...
function fixUnsupportedEdgeStyles(source, astEdges, astNodes) {
    ...
    const wrongNodeToCorrect = new Map();
    // 修正 AST 边
 ...
 * 宽容模式：即使有错误也返回部分结果
 */
export function parseMermaid(source, errorCollector) {
    ...
    // 解析节点
    const nodes = [];
    const nodeIdMap = new Map(); // 原始ID → 画布ID
    let nodeIndex = 0;
    if (astNodes) {
        for (const [originalId, astNode] of astNodes) {
            const shape = SHAPE_MAP[astNode.shape] ?? 'rect';
            const canvasId = idGen.next();
