# @mermaid2aichat/server

Mermaid2AIChat 的 MCP 服务端。通过 MCP 协议（Streamable HTTP）向 AI 暴露画布读写工具，并通过 WebSocket 与可视化编辑器（Web 编辑器 / VSCode 插件）双向同步画布状态。
...
| `/health` | GET | 健康检查，返回 `{ "status": "ok", "service": "mermaid2aichat" }` |
...
```typescript
import { startServer } from '@mermaid2aichat/server';
...
```bash
pnpm --filter @mermaid2aichat/server install   # 安装依赖（monorepo 根目录）
pnpm --filter @mermaid2aichat/server dev       # 开发模式（tsx watch 热重载）
pnpm --filter @mermaid2aichat/server build     # 构建（tsup，输出 ESM + dts）
pnpm --filter @mermaid2aichat/server start     # 生产启动
pnpm --filter @mermaid2aichat/server typecheck # 类型检查
pnpm --filter @mermaid2aichat/server test      # 运行测试（vitest）
```
...
| `zustand` | 画布状态 Store |
| `zod` | 工具参数校验 |
| `@mermaid2aichat/serializer` | Mermaid 解析/序列化（workspace 依赖） |
