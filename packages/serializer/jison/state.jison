 ...
 *
 *  移植自官方 mermaid stateDiagram.jison，支持 stateDiagram-v2 完整语法：
 *  - 状态声明（id / "label" as id / [*] start/end）
 *  - 状态类型（<<fork>> / <<join>> / <<choice>> / [[fork]] / [[join]] / [[choice]]）
 *  - 转换关系（-->）
 *  - 复合状态（state X { ... }）
 *  - 并发区域（--）
 *  - Note（note left of / note right of / note "text" as id）
...
<INITIAL,ID,STATE,struct,LINE>\%\%(?!\{)[^\n]*          /* skip comments */
"scale"\s+            { this.pushState('SCALE'); return 'scale'; }
...
<STATE>.*"<<fork>>"                   {this.popState();yytext=yytext.slice(0,-8).trim();return 'FORK';}
<STATE>.*"<<join>>"                   {this.popState();yytext=yytext.slice(0,-8).trim();return 'JOIN';}
<STATE>.*"<<choice>>"                 {this.popState();yytext=yytext.slice(0,-10).trim();return 'CHOICE';}
<STATE>.*"[[fork]]"                   {this.popState();yytext=yytext.slice(0,-8).trim();return 'FORK';}
<STATE>.*"[[join]]"                   {this.popState();yytext=yytext.slice(0,-8).trim();return 'JOIN';}
<STATE>.*"[[choice]]"                 {this.popState();yytext=yytext.slice(0,-10).trim();return 'CHOICE';}
...
<STATE>["]                 { this.pushState("STATE_STRING"); }
<STATE>\s*"as"\s+          { this.pushState('STATE_ID'); return "AS"; }
<STATE_ID>[^\n\{]*         { if (!processId()) return; this.popState(); return "ID"; }
<STATE_STRING>["]          { this.popState(); }
<STATE_STRING>[^"]*        { return "STATE_DESCR"; }
<STATE>\w+\s+\w+.*?\{      { throw new Error('Error: State name must be a single word. Found: "' + yytext.trim() + '"'); }
<STATE>[^\n\s\{]+          { return 'COMPOSIT_STATE'; }
<STATE>\n                  { this.popState(); }
<INITIAL,STATE>\{          { this.popState(); this.pushState('struct'); return 'STRUCT_START'; }
<struct>\}                 { this.popState(); return 'STRUCT_STOP'; }
<struct>[\n]               /* nothing */
...
<FLOATING_NOTE>["]                  /**/
<FLOATING_NOTE>[^"]*                { return "NOTE_TEXT"; }
<FLOATING_NOTE_ID>[^\n]*            { if (!processId()) return; this.popState(); return "ID"; }
<NOTE_ID>\s*[^:\n\s\-]+             { if (!processId()) return; this.popState(); this.pushState('NOTE_TEXT'); return 'ID'; }
<NOTE_TEXT>\s*":"[^:\n;]+           { this.popState(); yytext = yytext.substr(2).trim(); return 'NOTE_TEXT'; }
<NOTE_TEXT>[\s\S]*?\n\s*"end note"  { this.popState(); yytext = yytext.slice(0,-8).trim(); return 'NOTE_TEXT'; }

"stateDiagram"\s+                   { return 'SD'; }
"stateDiagram-v2"\s+                { return 'SD'; }

"hide empty description"      { return 'HIDE_EMPTY'; }

<INITIAL,struct>"[*]"                   { return 'EDGE_STATE'; }
<INITIAL,struct>[^:\n\s\-\{]+           { if (!processId()) return; return 'ID'; }
<INITIAL,struct>\s*":"(?:[^:\n;]|":"[^:\n;])+  { yytext = yytext.trim(); return 'DESCR'; }
