/** mermaid
 *  Gantt 甘特图 jison 文法（移植官方 mermaid v11）
 *
 *  来源: mermaid-develop/packages/mermaid/src/diagrams/gantt/parser/gantt.jison
 *
 *  修改说明（M8 移植版）:
 *    - 文法规则保持与官方一致，确保解析能力完整
 *    - 调用的 yy 方法由 GanttDB 移植版提供（packages/serializer/src/parser/gantt-db.ts）
 *    - weekday 支持 7 天（与官方一致），GanttDB 内部仅接受 sunday/monday（类型收窄）
 *
 *  MIT license.
 */
%lex
...
statement
  ...
  | topAxis {yy.enableTopAxis();$$=$1.substr(8);}
  | axisFormat {yy.setAxisFormat($1.substr(11));$$=$1.substr(11);}
  | tickInterval {yy.setTickInterval($1.substr(13));$$=$1.substr(13);}
  | displayMode {yy.setDisplayMode($1.substr(12).trim());$$=$1.substr(12).trim();}
